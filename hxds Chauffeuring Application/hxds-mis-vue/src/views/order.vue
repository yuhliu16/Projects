	...
	methods: {
		...
		loadPanelData: function(ref, row) {
			...
			ref.$http('order/searchOrderComprehensiveInfo', 'POST', data, true, function(resp) {
				...
				//更新表格row记录的数据，以免订单状态变更，row数据不一致
		...
		showUpdatePanel: function(id) {
			this.$nextTick(() => {
				this.$refs.orderClose.init(id);
			});
		},
		showOrderBill: function(id) {
			this.$nextTick(() => {
				this.$refs.orderBill.init(id);
			});
		}
	},
	created: function() {
